...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default="mt",
        choices=["mt", "dask-local", "dask-ssh", "dask-remote", "dask-slurm"],
    )
    p.add_argument(
        "--scheduler-address",
...
def create_dask_client(scheduler: str, ncores: int, hosts: str, scheduler_address: str) -> Client:
    ...
    if scheduler == "dask-slurm": #Modified for 2 nodes request
        cluster = SLURMCluster(
            local_directory = os.getcwd(),
            job_extra_directives=["--nodes=1","--partition=standard"],
            n_workers = 128,
            processes = 128,
            account = "project_465001202",
            cores =128,
            memory = "224 GB")
    ...
    raise ValueError(
        f"Unexpected scheduling mode '{scheduler}'. Valid modes are ['dask-local', 'dask-ssh', 'dask-remote', 'dask-slurm']."
    )
...
def main() -> None:
    ...
    else: ## here we need modifications mb?
