...
BASE_OUTPUT_DIR="$PWD/output_1node_2task_$(date +%Y%m%d_%H%M%S)"
...
srun --exclusive -N1 -n1 bash -c "$BASE_OUTPUT_DIR/worker.sh 1 ${datasets[0]} $BASE_OUTPUT_DIR" &
srun --exclusive -N1 -n1 bash -c "$BASE_OUTPUT_DIR/worker.sh 2 ${datasets[1]} $BASE_OUTPUT_DIR" &
