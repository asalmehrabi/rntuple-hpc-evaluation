...
base_output_dir="$PWD/flash_dir/performance_output_flash_srun$(date +%Y%m%d_%H%M%S)"
...
for cores in 128 64 32 16; do
    ...
    for run in {1..10}; do
        ...
        srun -n 1 -c $cores /usr/bin/time -v /users/mehrabia/AGC-venv/bin/python3 /users/mehrabia/AGC/analysis-grand-challenge/analyses/cms-open-data-ttbar/analysis.py --ncores $cores > $output_dir/output_run${run}.log 2> $output_dir/error_run${run}.log
